    ...
    <table class="tree">
      <tbody>
        ...
        <tr class="border-bottom">
          <td></td>
          <td class="th_total text-bold"><?php echo new_html_entity_decode($data_report['this_year_header']); ?></td>
          <td class="th_total text-bold"><?php echo new_html_entity_decode($data_report['last_year_header']); ?></td>
        </tr>
        <?php
          $row_index = 0;
          $parent_index = 0;
          ...
          ?>
          <tr class="treegrid-<?php echo new_html_entity_decode($parent_index); ?> parent-node expanded">
          ...
          <?php 
            ...
            $total_py_income = $data['total_py_amount'];
             ?>
          <?php $row_index += 1; ?>
          <tr class="treegrid-total-<?php echo new_html_entity_decode($row_index); ?> parent-node expanded tr_total">
          ...
          <?php 
            $_index = $row_index;
            $data = $this->accounting_model->get_html_profit_and_loss_comparison($data_report['data']['net_income'], ['html' => '', 'row_index' => $row_index + 1, 'total_amount' => 0, 'total_py_amount' => 0], $parent_index, $currency);
            $row_index = $data['row_index'];
            echo new_html_entity_decode($data['html']);
            $total_net_income = $data['total_amount'];
            $total_py_net_income = $data['total_py_amount'];
             ?>
          <?php $row_index += 1; ?>
          <tr class="treegrid-total-<?php echo new_html_entity_decode($row_index); ?> parent-node expanded tr_total">
            ...
            <td class="total_amount"><?php echo app_format_money($total_py_net_income, $currency->name); ?></td>
          </tr>
          <?php } ?>
          <?php if($acc_enable_income_statement_modifications != 1){ ?>
            ...
            <tr class="treegrid-total-<?php echo new_html_entity_decode($row_index); ?> parent-node expanded tr_total">
              ...
              <td class="total_amount"><?php echo app_format_money(($total_income + $total_net_income) - $total_cost_of_sales, $currency->name); ?> </td>
              <td class="total_amount"><?php echo app_format_money($total_py_income + $total_py_net_income - $total_py_cost_of_sales, $currency->name); ?> </td>
            </tr>
          <?php } ?>
          <?php $row_index += 1;
            $parent_index = $row_index;
          ...
          <tr class="treegrid-total-<?php echo new_html_entity_decode($row_index); ?> parent-node expanded tr_total">
            ...
            <td class="total_amount"><?php echo app_format_money(($total_py_income + $total_py_net_income + $total_py_other_income) - ($total_py_cost_of_sales + $total_py_expenses + $total_py_other_expenses), $currency->name); ?> </td>
          </tr>
        </tbody>
    </table>
  </div>
